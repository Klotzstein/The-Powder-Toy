...
int update_PYRO(UPDATE_FUNC_ARGS) {
	...
	if(t==PT_FIRE && t==PT_EMBE && parts[i].life <=1)
	...
	for (rx=-2; rx<3; rx++)
		for (ry=-2; ry<3; ry++)
			if (x+rx>=0 && y+ry>0 && x+rx<XRES && y+ry<YRES && (rx || ry))
			{
				...
				{
					part_change_type(r>>8,x+rx,y+ry,PT_FIRE || PT_EMBE);
                ...
                if (parts[i].type==PT_LAVA && t==PT_EMBE){
                    if (parts[r>>8].type==PT_LAVA && t==PT_EMBE){
	...
	for (rx=-2; rx<3; rx++)
		for (ry=-2; ry<3; ry++)
			if (x+rx>=0 && y+ry>0 && x+rx<XRES && y+ry<YRES && (rx || ry))
			{
				...
				if (lpv < 1) lpv = 1;
				if (t!=PT_SPRK && ptypes[rt].meltable  && ((rt!=PT_RBDM && rt!=PT_LRBD) || t!=PT_SPRK) && ((t!=PT_FIRE&&t!=PT_PLSM&&t!=PT_BFLM&&t!=PT_EMBE) || (rt!=PT_METL && rt!=PT_IRON && rt!=PT_ETRD && rt!=PT_PSCN && rt!=PT_NSCN && rt!=PT_NTCT && rt!=PT_PTCT && rt!=PT_BMTL && rt!=PT_BRMT && rt!=PT_SALT && rt!=PT_INWR)) &&
                    ptypes[rt].meltable*lpv>(rand()%1000))
				{
					if (t!=PT_LAVA && t!=PT_EMBE || parts[i].life>0)
					{
						parts[r>>8].ctype = (rt==PT_BRMT)?PT_BMTL:parts[r>>8].type;
						parts[r>>8].ctype = (parts[r>>8].ctype==PT_SAND)?PT_GLAS:parts[r>>8].ctype;
						part_change_type(r>>8,x+rx,y+ry,PT_LAVA && PT_EMBE);
